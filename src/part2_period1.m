...
filename = strcat(currdir,name);
data = importfile(filename);
%% Split dataset into 2 equal(based on time) periods
...
%% Cross correlation after whittening with method 1 (AR)
for indx1=1:m-1
    for indx2=indx1+1:m
        y1V = yM(:,indx1);
        y2V = yM(:,indx2);
...
%}
%% Autocorrelation after whittening with method 2 (log)
%{
for indx1=1:m
    y1V = yM(:,indx1);
...
%}
%%  Cross correlation after whittening with method 2 (log) 
%{
i=1
for indx1=1:m-1
    for indx2=indx1+1:m
